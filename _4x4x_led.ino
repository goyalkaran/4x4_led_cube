...
void loop()
{
  ...
  turnEverythingOn();
  delay(time);
  OnAndOffAllByLayerUpAndDownNotTimed();
  layersUpAndDown();
  spiralInAndOut();
  OnAndOffAllByColumnSideways();
  delay(time);
  aroundEdgeDown();
  turnEverythingOff();
  randomflicker();
  ...
  turnEverythingOff();
  delay(time);
  movingWavefrontFirst();
  movingWavefrontSecond();
  increasingCube();
  turnEverythingOff();
  delay(2000);
}
...
void OnAndOffAllByLayerUpAndDownNotTimed()
{
  int x = 75;
  for (int i = 5; i != 0; i--)
  {
...
void OnAndOffAllByColumnSideways()
{
  int x = 75;
  turnEverythingOff();
  //turn on layers
...
void randomLed()
{
  ...
  for (int i = 1; i != 200; i++)
  {
    ...
    digitalWrite(layer[randomLayer], 1);
    delay(30);
    //stop led's by layer and column
    digitalWrite(column[randomColumn], 1);
    digitalWrite(layer[randomLayer], 0);
